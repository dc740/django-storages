...
class LibCloudStorage(Storage):
    ...
    def get_default_settings(self):
        if provider_name is None:
            provider_name = setting("DEFAULT_LIBCLOUD_PROVIDER", "default")
        ...
        return {
            ...
            "option": setting("LIBCLOUD_OPTION", None),
            "cloudfiles_url_expires": setting("LIBCLOUD_CLOUDFILES_URL_EXPIRES", None),
        }
    ...
    def url(self, name, expire=None, http_method="GET"):
        provider_type = self.provider["type"].lower()
        obj = self._get_object(name)
        if not obj:
            return None
        if expire is None and self.cloudfiles_url_expires is not None:
            expire = self.cloudfiles_url_expires

        if expire is not None and "cloudfiles" in provider_type:
            url = self.driver.ex_get_object_temp_url(obj, http_method, expire)
        try:
